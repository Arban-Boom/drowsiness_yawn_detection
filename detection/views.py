...
# @login_required
# def run_drowsiness_detection(request):
#     subprocess.Popen([sys.executable, 'detection/tasks.py'])
#     return render(request, 'detection/home.html', {'message': 'Drowsiness detection started'})
        ...
        if form.is_valid():
            ...
            if authenticated_user is not None:
                ...
                if DriverProfile.objects.filter(license_number=license_number).exists():
                    messages.error(
                        request, 'License number already exists. Please use a different one.')
...
def update_settings(request):
    if request.method == "POST":
        ...
        ear_threshold = request.POST.get(
